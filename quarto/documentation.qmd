...
-   It costs our teams valuable time to manually intervene in the marketplace, AKA "boosting"

-   Boosting trips is expensive! As we try to guarantee every ride, costs accumulate on unclaimed rides leading up to the scheduled trip
...
-   Between May - August, the company paid out \$430k to drivers

-   The avg. trip was 21 minutes; avg. fare was \$25
...
Looking closely at the timeline of driver claims versus trips' scheduled start...
...
-   Many trips are claimed weeks in advance, but some just days or hours before. A third (33.2%) of trips remain unclaimed an hour before pick-up!

## The Data (features)

We know **fares, boosts, claimed/scheduled ride times**, and have also collected information on:
...
-   If we can probabilistically identify rides that are most likely to remain unclaimed until late, we can promote them on the platform early – saving on last-minute boosting.

## A model-based approach to the business problem

In developing a model, we have three high-level strategic goals:
...
I fit a predictive model to measure the extent to which our data inputs influenced each claimed trip's timeline

-   Let's start with a relatively simple linear model (GLM)

-   Specifically, what is the relationship between our features and the time between a trip getting claimed relative to its scheduled start, in hours?
...
-   Yet, we observe an asymmetric distribution and a high standard deviation among residuals (sd = 149 h), suggesting distributional assumptions are not holding up

-   In brief, my toy model's fit and performance leave much to be desired
...
-   It was insufficient to accommodate this degree of non-normality, evidenced by the right-skew
...
-   If this were a real deliverable, we would consider:
...
-   Both geographic-level features tracking monthly rides & commute times are modestly associated with later claims
...
-   **Adding an "early claim" bonus** for drivers, e.g. every five rides claimed 8+ hours early
...
-   **Same-day boosting costs the business \$5.27 per trip**, on average
...
-   **Automating early boosting for these trips will save the business time and money**
    ...
    -   Put another way, **a boost reduction of \$1 per \$25 trip saves us 4% on every fare**
...
-   We're assuming knowledge on fares paid to drivers ahead of time. Shifts in the predicted trip length/duration could systematically impact model results

-   **YMMV**: we need a way to quantify and track the value-add for the business once implementated

## Limitations ("out-of-sample")

-   We've operated based on "within-sample" data, and want to understand how well results would generalize to other times of the year, metro areas, and future trips in general
